    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-2">
            ...
            <button
                ...
                class="h-[32px] px-4 border border-red-200 rounded-lg text-[13px] font-medium text-red-400 hover:text-red-600 hover:border-red-300 flex items-center transition-colors">
                Delete
            </button>
        </div>
    </div>
    ...
    <div class="bg-white border border-black/[0.07] rounded-xl overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-black/[0.06]">
            <div class="flex items-start justify-between gap-4">
                ...
                @endphp
                <span class="shrink-0 inline-flex items-center h-[22px] px-2.5 rounded-md text-[11px] font-medium {{ $s['class'] }}">
                    {{ $s['label'] }}
                </span>
            </div>
            @if ($task->description)
        ...
        <div class="grid grid-cols-3 divide-x divide-black/[0.05]">
            ...
            <div class="px-6 py-4">
                ...
                <div class="text-[13px] font-medium text-gray-900">
                    @if ($task->due_date)
                        <span @class([
                            'text-red-500' => $task->due_date->isPast() && $task->status !== 'completed',
                        ])>
