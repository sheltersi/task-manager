<div class="my-6 mx-4 md:mx-[100px]">
    <div>
        ...
        <div class="grid grid-cols-4 gap-2.5 mb-6">
            @foreach ([['label' => 'Total', 'value' => $this->stats['total']], ['label' => 'In progress', 'value' => $this->stats['in_progress']], ['label' => 'In review', 'value' => $this->stats['in_review']], ['label' => 'Completed', 'value' => $this->stats['completed']]] as $stat)
                <div class="bg-white border border-black/[0.07] rounded-xl p-4">
                    ...
                    <div class="text-[22px] font-medium font-mono tracking-tight text-gray-900">
                        {{ str_pad($stat['value'], 2, '0', STR_PAD_LEFT) }}</div>
                </div>
            @endforeach
        </div>
        ...
        {{-- Toolbar --}}
        <div
            class="sticky top-0 z-10 bg-white/80 backdrop-blur border border-black/[0.06] rounded-xl p-3 shadow-sm flex flex-wrap items-center gap-2 mb-4">
        ...
        @endif
        {{-- Column headers --}}
        <div class="flex items-center justify-between gap-4 px-4 pb-1 mb-0.5">
            <div class="flex-1"></div>
            <div class="flex items-center gap-6">
                <span
                    class="w-[110px] text-center text-[10px] uppercase tracking-widest font-medium text-gray-400">Status
                    select</span>
                <span
                    class="w-[72px] text-center text-[10px] uppercase tracking-widest font-medium text-gray-400">Status</span>
                <span
                    class="w-[36px] text-center text-[10px] uppercase tracking-widest font-medium text-gray-400">Priority</span>
                <span
                    class="w-[180px] text-right pr-3 text-[10px] uppercase tracking-widest font-medium text-gray-400">Actions</span>
            </div>
        </div>

        <div class="border-t border-black/[0.05] mb-2.5"></div>
        {{-- Task List --}}
        <div class="space-y-3">
                ...
                <div class="group bg-white border border-black/[0.06] rounded-xl p-4 hover:shadow-md transition-all">
                    ...
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-6">
                            ...
                            <span class="w-[72px] flex justify-center">
                                @switch($task->status)
                                    @case('to_do')
                                        <span class="bg-gray-100 text-gray-500 px-2 py-1 rounded-md text-xs font-medium">To
                                            do</span>
                                    @break

                                    @case('in_progress')
                                        <span class="bg-[#E6F1FB] text-[#0C447C] px-2 py-1 rounded-md text-xs font-medium">In
                                            progress</span>
                                    @break

                                    @case('in_review')
                                        <span class="bg-[#EEEDFE] text-[#3C3489] px-2 py-1 rounded-md text-xs font-medium">In
                                            review</span>
                                    @break

                                    @case('completed')
                                        <span
                                            class="bg-[#EAF3DE] text-[#27500A] px-2 py-1 rounded-md text-xs font-medium">Completed</span>
                                    @break

                                    @default
                                        <span
                                            class="bg-gray-100 text-gray-500 px-2 py-1 rounded-md text-xs font-medium">Unknown</span>
                                @endswitch
                            </span>
                            {{-- Priority Badge --}}
                            <span
                                class="text-xs font-semibold px-2 py-1 rounded-md w-[36px] flex justify-center
                            ...
                            {{-- Actions --}}
                            <div
                                class="mw-[150px] justify-endflex items-center gap-12 opacity-70 group-hover:opacity-100 transition">
                                <a href="{{ route('tasks.show', $task) }}" wire:navigate
                               ...
                               <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none"
                                viewBox="0 0 576 512"><!--!Font Awesome Free v7.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2026 Fonticons, Inc.--><path fill="green" d="
                                                M288 32c-80.8 0-145.5 36.8-192.6 80.6-46.8 43.5-78.1 95.4-93 131.1-3.3
                                                7.9-3.3 16.7 0 24.6 14.9 35.7 46.2 87.7 93 131.1 47.1 43.7 111.8 80.6
                                                192.6 80.6s145.5-36.8 192.6-80.6c46.8-43.5 78.1-95.4 93-131.1 3.3-7.9
                                                3.3-16.7
                                                0-24.6-14.9-35.7-46.2-87.7-93-131.1-47.1-43.7-111.8-80.6-192.6-80.6zM144
                                                256a144 144 0 1 1 288 0 144 144 0 1 1 -288 0zm144-64c0 35.3-28.7 64-64
                                                64-11.5 0-22.3-3-31.7-8.4-1 10.9-.1 22.1 2.9 33.2 13.7 51.2 66.4 81.6
                                                117.6 67.9s81.6-66.4 67.9-117.6c-12.2-45.7-55.5-74.8-101.1-70.8 5.3 9.3
                                                8.4 20.1 8.4 31.7z" />
                                        </svg>
                                        View</span>
                                </a>
                                ...
                                <button wire:click="delete({{ $task->id }})"
                                    wire:confirm="Are you sure you want to delete the '{{ addslashes($task->title) }}' task?"
                                    class="ml-12 flex items-center justify-center rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500">
                                    <span class="flex items-center gap-1">
